...
int main(void)
{
   ...
   /*sprintf(str1, " buf1 %x  buf2  %x  val  %d  \n" ,i2cbuf[0],i2cbuf[1], pwmServos[5]);
    ...
    {
 // Checks the command and puts data in apropriate variable //
       // Velocity in X
        if( command == 0) 
        { 
            if( dataSign == 1)
...
// If commans is 0xf and there is new data
 if ( command == 0xf && dataReady == 1 )
    ...
    // Dynamics Math ( sefunctions bellow//
    body2wheel_vel(VxIn, VyIn, OmegaIn);
    
    
    
    // Write PWM to Servos and Print to UART //
    PWM1_WriteCompare(pwmServos[0]); PWM2_WriteCompare(pwmServos[1]); PWM3_WriteCompare(pwmServos[2]); 
    PWM4_WriteCompare(pwmServos[3]); PWM5_WriteCompare(pwmServos[4]); PWM6_WriteCompare( pwmServos[5] ); 
     sprintf(str1,"servo1  %d \n servo2  %d \n servo3  %d \n servo4  %d \n servo5  %d \n servo6 %d  \n ",pwmServos[0],pwmServos[1],pwmServos[2],pwmServos[3],pwmServos[4], pwmServos[5]);
    UART_PutString(str1 );  
        
    UART_PutString(" PWM Recieved \n");
...
void body2wheel_vel(float vx, float vy, float w){
    ...
    // left and rotatign counter-clockwise
    for (int i = 0; i<6; i++) {
...
void ind_wheel_spd_ang(float vx, float vy, float rx, float ry, float w){
    ...
    // this deales with undefined angles when vx=0
    if( v_wheelx > 0.01 ){
        spd_ang[1] = atanf(v_wheely/v_wheelx); // Wheel Angle
    }
    else if (v_wheely<0){
